...
Texture2D    bodynormaltexture : register(t1);
SamplerState bodynormalsampler : register(s1);

Texture2D    normaltexture : register(t2);
SamplerState normalsampler : register(s2);

Texture2D    maskedtexture : register(t3);
SamplerState maskedsampler : register(s3);
...
{
    float4 baseTexture = basetexture.Sample(basesampler, input.uv1);
    float3 baseTextureColor = baseTexture.xyz;

    float4 bodyNormal = bodynormaltexture.Sample(bodynormalsampler, input.uv0);
    float4 clothNormal = normaltexture.Sample(normalsampler, input.uv1);

    float3 NormalColor = (bodyNormal.xyz * 0.45f + clothNormal.xyz * 0.45f + input.normalEye * 0.05f);

    float4 bodyMasked = maskedtexture.Sample(maskedsampler, input.uv1);
    float3 bodyMaskedColor = bodyMasked.xyz;

    float3 fragToCamDir = normalize(-input.posEye);

    //clip(baseTexture.w - 0.05f);
    ...
    for (i = 0; i < pointLightNum; ++i)
    {
        ...
        float diffuseFactor = max(0.0, dot(NormalColor, lightDirEye));
        float3 iDiffuse = diffuseFactor;

        float3 halfwayEye = normalize(fragToCamDir + lightDirEye);
        float specularFactor = max(0.0, dot(halfwayEye, NormalColor));
        float3 iSpecular =
    ...
    float3 result = (dirLightIntensity + pointLightIntensity) * (float3(1.0f, 0.71f, 0.677f) * bodyMaskedColor.g);
    result += (dirLightIntensity + pointLightIntensity) * (baseTextureColor * (1.0f - bodyMaskedColor.g));

    result *= 2.0f;

    float normalDotView = saturate(dot(input.normalEye, input.cameraPos.xyz));

    float f0 = pow(1.0f - normalDotView, 5.0);
    float4 FresnelR0 = float4(0.3, 0.3, 0.3, 0.0);

    return float4(result, 1.0f) + (FresnelR0 * f0);
}
