...
float4 ps_main(VS_Output input) : SV_Target
{
    ...
    float3 NormalColor = input.normalEye;
    ...
    float3 result = (dirLightIntensity + pointLightIntensity) * float3(0.2f, 0.2f, 0.2f);

    float normalDotView = saturate(dot(input.normalEye, input.cameraPos.xyz));

    float f0 = pow(1.0f - normalDotView, 2.0);
    float4 FresnelR0 = float4(0.2, 0.2, 0.2, 0.0);
